...
function updateOctory () {
    ...
    if [[ -a "/Library/Application Support/Octory" ]]; then
        ...
        if [[ $(ps aux | grep -i "Octory" | grep -v grep) ]]; then
            ...
            case "$1" in
                installing) updateSplashScreen wait "Installing" ;;
                installed)  updateSplashScreen success "Installed" ;;
                failed)     updateSplashScreen fail "Failed" ;;
                notInstalled) updateSplashScreen pending "Not installed" ;;
            esac
        fi
    fi
