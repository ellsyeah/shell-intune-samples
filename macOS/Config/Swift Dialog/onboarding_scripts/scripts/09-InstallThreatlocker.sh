...
is_threatlocker_profiles_ready() {
  ...
  updateSplashScreen pending "Waiting for profiles"
  return 2
}
...
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        if [[ "$state" == *"ACTIVE_ENABLED"* ]]; then
            ...
            updateSplashScreen fail "User approval required"
			cleanup
            exit 1
        else
            echo "ThreatLocker installed but in an unexpected state. System extension state: $state"
            updateSplashScreen fail "Unexpected state"
			cleanup
            exit 1
        fi
    else
